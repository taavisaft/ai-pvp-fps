...
- Protocol v3 and world revision live in `src/protocol.h`. Snapshots contain all 16 player slots plus an occupied-slot mask and up to 64 bullet records; only the bullet tail is truncated. Simulation has 256 bullet slots.
...
- Client prediction currently smooths position corrections; it does not replay acknowledged/unacknowledged input history. Shot cadence and packet validation still need server hardening.
...
| Server / transport / wire data | `src/server_main.cpp`, `src/network.h/.cpp`, `src/net_common.h/.cpp`, `src/platform.h`, `src/protocol.h` |
...
- Add focused headless regression proof for physics/network changes; no automated test suite is wired into CMake yet.
